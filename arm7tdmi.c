...
void arm7_reset()
{
    ...
    size = ftell(f);
    fseek(f, 0, SEEK_SET);
    fread(&testmem[0x8000000], 1, size, f);
    fclose(f);
    reg(0) = 0x08000109;
    reg(13) = 0x03007F00;
...
{
    ptr &= 0xFFFFFFFE;
    return testmem[ptr] + (testmem[ptr + 1] << 8);
}

uint32_t arm7_read(uint32_t ptr)
{
    ptr &= 0xFFFFFFFC;
    return testmem[ptr] + (testmem[ptr + 1] << 8) + (testmem[ptr + 2] << 16) + (testmem[ptr + 3] << 24);
}
...
{
    ptr &= 0xFFFFFFFE;
    testmem[ptr] = value & 0xFF;
    testmem[ptr + 1] = value >> 8;
}
...
void arm7_write(uint32_t ptr, uint32_t value)
{
    ptr &= 0xFFFFFFFC;
    testmem[ptr] = value & 0xFF;
    testmem[ptr + 1] = (value >> 8) & 0xFF;
    testmem[ptr + 2] = (value >> 16) & 0xFF;
    testmem[ptr + 3] = value >> 24;
}
